...
import HSP                        (XMLGenerator, XMLType, XMLGenT(..), Attr(..), EmbedAsAttr(..), EmbedAsChild(..), SetAttr, GenXML, (<@), genElement, genEElement, unXMLGenT)
...
type FormDF m a = Form m [Input] String [XMLGenT m (XMLType m)] a
...
rowsToList :: (XMLGenerator x, EmbedAsChild x v, Functor m, Monad m, Monoid a) => String -> [Form m i e v a] -> Form m i e [XMLGenT x (XMLType x)] a
...
rowsToColumn :: (XMLGenerator x, Applicative m, Monad m, Monoid a) => [Form m i e [XMLGenT x (XMLType x)] a] -> Form m i e [XMLGenT x (XMLType x)] a
